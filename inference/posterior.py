...
class Posterior:
    ...
    @torch.no_grad()
    def elbo_sample(self) -> torch.Tensor:
        """Returns the Evidence Lower Bound associated to each sample.
            This function was added by Martin Treppner.
        """
        elbo_sample = compute_elbo_sample(self.model, self)
        return elbo_sample
    ...
    def generate(
        ...
        for tensors in self.update({"batch_size": batch_size}):
            ...
            outputs = self.model.inference(
                sample_batch, batch_index=batch_index, y=labels, n_samples=n_samples, sample_prior=sample_prior, n_cells=n_cells
            )
            px_r = outputs["px_r"]
